...
public final class EconomyMarketModule implements CoreModule, DiscordCommandContributor {
    ...
    private void handleBalance(SlashCommandInteractionEvent event) {
        ...
        event.deferReply(ephemeral).queue(hook -> resolveTarget(event, false)
                .thenCompose(target -> plugin.callSync(() -> {
                    OfflinePlayer player = Bukkit.getOfflinePlayer(target.uuid());
                    double balance = vault.balance(player);
                    return balanceEmbed(target, balance);
                }))
    ...
    private CompletableFuture<PlayerTarget> resolveTarget(
        ...
        if (!requested.isBlank()) {
            ...
            return plugin.callSync(() -> {
                ...
                if (online != null) return new PlayerTarget(online.getUniqueId(), online.getName());
                if (mustBeOnline) throw new IllegalStateException(
                        "That player is offline. Inventory snapshots are only available while a player is online.");
                @SuppressWarnings("deprecation")
                OfflinePlayer offline = Bukkit.getOfflinePlayer(requested);
                if (!offline.hasPlayedBefore()) throw new IllegalStateException("No known player matches that name.");
                return new PlayerTarget(offline.getUniqueId(), Objects.requireNonNullElse(offline.getName(), requested));
            });
        }
        if (!requireLinked) {
