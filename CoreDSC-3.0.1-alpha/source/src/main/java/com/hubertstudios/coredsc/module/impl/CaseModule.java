...
public final class CaseModule implements CoreModule, DiscordCommandContributor {
    ...
    private void registerMinecraftCommands() {
        ...
        caseCommand.setExecutor((sender, command, label, args) -> {
            ...
            switch (args[0].toLowerCase(Locale.ROOT)) {
                case "view" -> { Long id=parseId(args[1]); if(id==null){sender.sendMessage("§cInvalid ID.");break;} cases.findCase(id).whenComplete((found,error)->plugin.runForSender(sender,()->sender.sendMessage(error!=null||found.isEmpty()?"§cCase not found.":formatCase(found.get())))); }
                case "player" -> cases.findCasesForTarget(args[1], 10).whenComplete((list,error)->plugin.runForSender(sender,()->{if(error!=null){sender.sendMessage("§cLookup failed.");return;}if(list.isEmpty()){sender.sendMessage("§7No cases found.");return;}sender.sendMessage("§bCases for "+args[1]+":");for(ModerationCase c:list)sender.sendMessage("§7- §f#"+c.id()+" §c"+c.action()+" §8["+c.status()+"] §7"+c.reason());}));
                case "close" -> { if(!sender.hasPermission("coredsc.case.manage")){sender.sendMessage("§cNo permission.");break;}Long id=parseId(args[1]);if(id==null){sender.sendMessage("§cInvalid ID.");break;}cases.updateCaseStatus(id,"CLOSED",System.currentTimeMillis()).whenComplete((ok,error)->plugin.runForSender(sender,()->sender.sendMessage(error==null&&ok?"§aCase closed.":"§cCase not found."))); }
        ...
        appealCommand.setExecutor((sender, command, label, args) -> {
            ...
            switch (args[0].toLowerCase(Locale.ROOT)) {
                case "create" -> {
                    ...
                    createAppeal(player.getUniqueId(),caseId,join(args,2)).whenComplete((result,error)->plugin.runForEntity(player,()->player.sendMessage(error==null?"§aAppeal #"+result+" created.":"§c"+rootMessage(error))));
                }
                case "status" -> {
                    if(args.length>=2){Long id=parseId(args[1]);if(id==null){player.sendMessage("§cInvalid appeal ID.");break;}cases.findAppeal(id).whenComplete((found,error)->plugin.runForEntity(player,()->{
                    ...
                    else cases.findAppealsForUser(player.getUniqueId().toString()).whenComplete((list,error)->plugin.runForEntity(player,()->{if(error!=null){player.sendMessage("§cLookup failed.");return;}if(list.isEmpty()){player.sendMessage("§7No appeals.");return;}for(Appeal a:list)player.sendMessage("§7Appeal §f#"+a.id()+" §7for case #"+a.caseId()+" §8["+a.status()+"]");}));
                }
    ...
    private void deliverOffline(Player player){
        messages.pendingForMinecraft(player.getUniqueId().toString(),"APPEAL",50).whenComplete((pending,error)->{
            ...
            plugin.runForEntity(player,()->{List<Long>ids=new ArrayList<>();for(SupportMessage message:own){player.sendMessage("§b[Appeal #"+message.itemId()+"] §f"+message.message());ids.add(message.id());}messages.markMinecraftDelivered(ids);});
        });
    }
