...
public final class ApplicationModule implements CoreModule, DiscordCommandContributor {
    ...
    private void registerMinecraftCommands(){
        ...
        staff.setExecutor((sender,cmd,label,args)->{if(!sender.hasPermission("coredsc.application.manage")){sender.sendMessage("§cNo permission.");return true;}if(args.length<2){sender.sendMessage("§e/application <view|accept|reject> <id> [note]");return true;}Long id=parseId(args[1]);if(id==null){sender.sendMessage("§cInvalid ID.");return true;}switch(args[0].toLowerCase(Locale.ROOT)){
            case "view"->applications.find(id).thenCombine(applications.answers(id),(app,answers)->app.map(a->format(a,answers)).orElse("Application not found.")).whenComplete((text,error)->plugin.runForSender(sender,()->sender.sendMessage(error==null?text:"§cLookup failed.")));
            case "accept","reject"->{boolean accept=args[0].equalsIgnoreCase("accept");String note=args.length>2?join(args,2):"";decide(id,accept,sender.getName(),note).whenComplete((ok,error)->plugin.runForSender(sender,()->sender.sendMessage(error==null&&ok?"§aApplication decided.":"§cApplication not found or already decided.")));}
            default->sender.sendMessage("§e/application <view|accept|reject> <id> [note]");
        }return true;});
    }

    private void start(Player p){String playerUuid=p.getUniqueId().toString();String playerName=p.getName();links.findByMinecraftUuid(playerUuid).thenCompose(link->{if(requireLinked&&link.isEmpty())return CompletableFuture.failedFuture(new IllegalStateException("Link your Discord account first."));return applications.start(playerUuid,playerName,link.map(LinkedAccountRepository.LinkedAccount::discordUserId).orElse(""),System.currentTimeMillis());}).whenComplete((id,error)->plugin.runForEntity(p,()->{if(error!=null){p.sendMessage("§c"+rootMessage(error));return;}p.sendMessage("§aApplication #"+id+" started. Use §e/apply questions§a and §e/apply answer <id> <answer>§a.");}));}
    ...
    }).whenComplete((v,e)->plugin.runForEntity(p,()->p.sendMessage(e==null?"§aApplication submitted.":"§c"+rootMessage(e))));}
    private void status(Player p){applications.activeForUser(p.getUniqueId().toString()).whenComplete((app,e)->plugin.runForEntity(p,()->p.sendMessage(e!=null?"§cLookup failed.":app.isEmpty()?"§7No active application.":"§bApplication #"+app.get().id()+" §8["+app.get().status()+"]")));}
    ...
    private void deliverOffline(Player player){
        messages.pendingForMinecraft(player.getUniqueId().toString(),"APPLICATION",50).whenComplete((pending,error)->{
            ...
            plugin.runForEntity(player,()->{List<Long>ids=new ArrayList<>();for(SupportMessage message:own){player.sendMessage("§b[Application #"+message.itemId()+"] §f"+message.message());ids.add(message.id());}messages.markMinecraftDelivered(ids);});
        });
    }
