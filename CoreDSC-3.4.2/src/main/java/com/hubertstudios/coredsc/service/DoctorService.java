...
public final class DoctorService {
    ...
    public void diagnose(CommandSender sender) {
        ...
        queueFuture.handle((counts, error) -> new AsyncResult(counts, error))
                ...
                .whenComplete((results, combinationError) -> replyTarget.execute(currentSender -> {
    ...
    public void fix(CommandSender sender) {
        ...
        if (queue != null) queue.retryFailed().whenComplete((count, error) -> replyTarget.send(
        ...
        if (plugin.getStorage() != null) new PendingLinkCodeRepository(plugin.getStorage())
                .deleteExpired(System.currentTimeMillis()).whenComplete((count, error) -> replyTarget.send(
    ...
    private void testChannel(CommandSender sender, String channelId, String message) {
        ...
        if (queue != null) {
            queue.enqueue(channelId, message, 100, "doctor:" + channelId + ':' + System.currentTimeMillis())
                    .whenComplete((id, error) -> replyTarget.send(error == null
        ...
        channel.sendMessage(message).queue(ignored -> replyTarget.send("§aTest message sent."),
                error -> replyTarget.send("§cTest failed: " + rootMessage(error)));
    }
