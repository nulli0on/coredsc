...
public final class CaseModule implements CoreModule, DiscordCommandContributor {
    ...
    public void enable() {
        ...
        bukkitListener = new Listener() {
            @EventHandler public void onJoin(PlayerJoinEvent event) {
                deliverOffline(event.getPlayer().getUniqueId());
            }
        };
    ...
    /**
     * Returns detached primitives for the hosted dashboard. Repository and
     * Bukkit objects never cross the asynchronous cloud boundary.
     */
    public CompletableFuture<List<Map<String, Object>>> cloudHistory(String target, int limit) {
        String normalized = target == null ? "" : target.trim();
        if (normalized.isBlank() || normalized.length() > 64) {
            return CompletableFuture.failedFuture(new IllegalArgumentException(
                    "target must contain a Minecraft name or UUID and be at most 64 characters"));
        }
        CaseRepository repository = cases;
        if (repository == null) {
            return CompletableFuture.failedFuture(new IllegalStateException("The cases module is not ready"));
        }
        int boundedLimit = Math.max(1, Math.min(limit, 50));
        return repository.findCasesForTarget(normalized, boundedLimit).thenApply(found -> found.stream().map(value -> {
            Map<String, Object> row = new LinkedHashMap<>();
            row.put("id", value.id());
            row.put("action", safeCaseValue(value.action()));
            row.put("targetUuid", safeCaseValue(value.targetUuid()));
            row.put("targetName", safeCaseValue(value.targetName()));
            row.put("executor", safeCaseValue(value.executor()));
            row.put("reason", safeCaseValue(value.reason()));
            row.put("duration", safeCaseValue(value.duration()));
            row.put("source", safeCaseValue(value.source()));
            row.put("status", safeCaseValue(value.status()));
            row.put("createdAt", value.createdAt());
            row.put("updatedAt", value.updatedAt());
            return Map.copyOf(row);
        }).toList());
    }

    private static String safeCaseValue(String value) {
    ...
    private void registerMinecraftCommands() {
        ...
        caseCommand.setExecutor((sender, command, label, args) -> {
            ...
            switch (args[0].toLowerCase(Locale.ROOT)) {
                case "view" -> { Long id=parseId(args[1]); if(id==null){sender.sendMessage("§cInvalid ID.");break;} cases.findCase(id).whenComplete((found,error)->replyTarget.send(error!=null||found.isEmpty()?"§cCase not found.":formatCase(found.get()))); }
                case "player" -> { String targetName=args[1]; cases.findCasesForTarget(targetName, 10).whenComplete((list,error)->replyTarget.execute(currentSender->{if(error!=null){currentSender.sendMessage("§cLookup failed.");return;}if(list.isEmpty()){currentSender.sendMessage("§7No cases found.");return;}currentSender.sendMessage("§bCases for "+targetName+":");for(ModerationCase c:list)currentSender.sendMessage("§7- §f#"+c.id()+" §c"+c.action()+" §8["+c.status()+"] §7"+c.reason());})); }
                case "close" -> { if(!sender.hasPermission("coredsc.case.manage")){sender.sendMessage("§cNo permission.");break;}Long id=parseId(args[1]);if(id==null){sender.sendMessage("§cInvalid ID.");break;}cases.updateCaseStatus(id,"CLOSED",System.currentTimeMillis()).whenComplete((ok,error)->replyTarget.send(error==null&&ok?"§aCase closed.":"§cCase not found.")); }
        ...
        appealCommand.setExecutor((sender, command, label, args) -> {
            ...
            switch (args[0].toLowerCase(Locale.ROOT)) {
                case "create" -> {
                    ...
                    createAppeal(playerId,caseId,join(args,2)).whenComplete((result,error)->
                            plugin.runForPlayer(playerId,currentPlayer->currentPlayer.sendMessage(
                                    error==null?"§aAppeal #"+result+" created.":"§c"+rootMessage(error))));
                }
                case "status" -> {
                    if(args.length>=2){Long id=parseId(args[1]);if(id==null){player.sendMessage("§cInvalid appeal ID.");break;}cases.findAppeal(id).whenComplete((found,error)->plugin.runForPlayer(playerId,currentPlayer->{
                        if(error!=null||found.isEmpty()||!found.get().minecraftUuid().equals(playerId.toString())) currentPlayer.sendMessage("§cAppeal not found.");
                        else currentPlayer.sendMessage(formatAppeal(found.get()));
                    }));}
                    else cases.findAppealsForUser(playerId.toString()).whenComplete((list,error)->plugin.runForPlayer(playerId,currentPlayer->{if(error!=null){currentPlayer.sendMessage("§cLookup failed.");return;}if(list.isEmpty()){currentPlayer.sendMessage("§7No appeals.");return;}for(Appeal a:list)currentPlayer.sendMessage("§7Appeal §f#"+a.id()+" §7for case #"+a.caseId()+" §8["+a.status()+"]");}));
                }
    ...
    private CompletableFuture<Void> notifyAppealDecision(Appeal appeal,String status,String by,String note){
        ...
        UUID playerId=UUID.fromString(appeal.minecraftUuid());
        return messages.add("APPEAL",appeal.id(),"SYSTEM","","CoreDSC",text,
                        System.currentTimeMillis(),false,true)
                .thenCompose(messageId->plugin.runForPlayer(playerId,player->
                                player.sendMessage("§b[Appeal #"+appeal.id()+"] §f"+text))
    ...
    private void deliverOffline(UUID playerId){
        messages.pendingForMinecraft(playerId.toString(),"APPEAL",50).whenComplete((pending,error)->{
            ...
            plugin.runForPlayer(playerId,currentPlayer->{List<Long>ids=new ArrayList<>();for(SupportMessage message:own){currentPlayer.sendMessage("§b[Appeal #"+message.itemId()+"] §f"+message.message());ids.add(message.id());}messages.markMinecraftDelivered(ids);});
        });
    }
