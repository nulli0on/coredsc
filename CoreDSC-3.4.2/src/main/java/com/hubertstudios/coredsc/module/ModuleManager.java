...
import com.hubertstudios.coredsc.module.impl.EconomyMarketModule;
...
public final class ModuleManager {
    ...
    public synchronized void loadEnabledModules() {
        ...
        List<ModuleSelection> selections = List.of(
                ...
                new ModuleSelection("economy-market", config.getBoolean("modules.economy-market", false),
                        () -> new EconomyMarketModule(plugin)),
                new ModuleSelection("lore-sync", config.getBoolean("modules.lore-sync", false),
                        () -> new LoreSyncModule(plugin)),
                new ModuleSelection("competitive", config.getBoolean("modules.competitive", false),
                        () -> new CompetitiveModule(plugin)),
                ...
                new ModuleSelection("web-editor", config.getBoolean("modules.web-editor", false),
                        () -> new WebEditorModule(plugin)),
                new ModuleSelection("python-bot", config.getBoolean("modules.python-bot", false),
                        () -> new PythonBotModule(plugin)),
                // Loaded last so the cloud coordinator can reference every
                // already-initialized feature module without owning it.
                new ModuleSelection("cloud-control", config.getBoolean("modules.cloud-control", false),
                        () -> new CloudControlModule(plugin))
        );
    ...
    /** Marks an asynchronously initialized module as failed after enable() returned. */
    public synchronized void markRuntimeFailed(String moduleId, String detail) {
        if (moduleId == null || moduleId.isBlank() || !enabledModules.containsKey(moduleId)) return;
        String safe = detail == null || detail.isBlank() ? "asynchronous initialization failed" : detail.trim();
        statuses.put(moduleId, new ModuleStatus(ModuleState.FAILED, safe));
        operationalStatuses.computeIfAbsent(moduleId, ignored -> new MutableOperationalStatus())
                .markFailure(safe);
    }
