 ...
 *
 * <p>No custom endpoint exists. The optional value in {@code license.yml}
 * remains local and is never validated or transmitted. Only the boolean state
 * "configured/not configured" can appear in a custom chart.</p>
 *
...
public final class MetricsService {
    ...
    private static final List<String> MODULE_IDS = List.of(
            ...
            "workflows", "authme", "voicechat-sync", "python-bot"
    );
    ...
    private volatile boolean licenseConfigured;
    ...
    public void start() {
        ...
        try {
            YamlConfiguration config = loadAndMigrate(file);
            licenseConfigured = isLicenseConfigured(config.getString("license", ""));
            if (!config.getBoolean("metrics.enabled", true)) {
                state = State.DISABLED;
                detail = "disabled in license.yml";
                return;
            }
            ...
            plugin.getLogger().info("[Metrics] Official bStats metrics started. "
                    + "Server owners can opt out in plugins/bStats/config.yml or license.yml.");
    ...
    public void stop() {
        ...
        if (state == State.RUNNING) {
            detail = "stopping with plugin lifecycle";
        }
    }
    ...
    private void refreshSnapshotNow() {
        if (!Bukkit.isPrimaryThread()) {
            throw new IllegalStateException("Metrics snapshots must be created on the server thread");
        }
        ...
        snapshot = new Snapshot(
                ...
                licenseConfigured ? "configured" : "not_configured",
    ...
    private void registerCharts(Metrics target) {
        ...
        target.addCustomChart(new SimplePie("license_identifier_configured", () -> snapshot.licenseState()));
    ...
    private YamlConfiguration loadAndMigrate(File file)
        ...
        changed |= setDefault(config, "license", "");
        changed |= setDefault(config, "metrics.enabled", true);

        // Remove the abandoned private-endpoint prototype if somebody tested a
        // pre-release source snapshot. CoreDSC must never contact that endpoint.
        if (config.contains("telemetry")) {
            config.set("telemetry", null);
            changed = true;
            plugin.getLogger().warning("[Metrics] Removed obsolete private telemetry settings from license.yml. "
                    + "CoreDSC now uses only official bStats metrics.");
        }
        if (config.contains("metrics.bstats-plugin-id")) {
            config.set("metrics.bstats-plugin-id", null);
            changed = true;
        }
        if (config.contains("metrics.distribution-channel")) {
            config.set("metrics.distribution-channel", null);
            changed = true;
        }
    ...
    private static boolean setDefault(YamlConfiguration config, String path, Object value) {
    ...
    private record Snapshot(
        ...
        private static Snapshot empty() {
            return new Snapshot(
                    ...
                    "unknown", "0", "other", "other", "other", "not_configured",
