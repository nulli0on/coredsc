...
public final class DeliveryQueueModule implements CoreModule {
    ...
    public void enable() {
        ...
        task = plugin.getServer().getScheduler().runTaskTimer(plugin, this::flush, 20L * 5L, 20L * interval);
    }
    ...
    public void flush() {
        ...
        repository.due(System.currentTimeMillis(), batchSize)
                ...
                .whenComplete((ignored, error) -> {
                    if (error != null) plugin.getLogger().warning("[DeliveryQueue] Flush failed: " + rootMessage(error));
                    flushing.set(false);
                });
    }
