...
/** Operational diagnostics without exposing tokens or license internals. */
public final class DoctorService {
    ...
    public void diagnose(CommandSender sender) {
        ...
        lines.add(line("Storage", plugin.getStorage() != null && plugin.getStorage().getState().name().equals("READY"),
                plugin.getStorage() == null ? "not initialised" : plugin.getStorage().getState().name()));
        ...
        com.hubertstudios.coredsc.metrics.MetricsService metrics = plugin.getMetricsService();
        boolean metricsHealthy = metrics != null
                && metrics.getState() != com.hubertstudios.coredsc.metrics.MetricsService.State.FAILED;
        lines.add(line("bStats metrics", metricsHealthy,
                metrics == null ? "not initialised" : metrics.getState() + " - " + metrics.getDetail()));
        if (modules != null && modules.hasFailedModules()) lines.add("§c  Failed: " + modules.failedModuleSummary());
        if (modules != null) {
            addModuleLine(lines, modules, "link");
            addModuleLine(lines, modules, "link-rewards");
            addModuleLine(lines, modules, "nickname-sync");
            addModuleLine(lines, modules, "booster-rewards");
            addModuleLine(lines, modules, "ban-sync");
            addModuleLine(lines, modules, "chat-sync");
            addModuleLine(lines, modules, "console");
            addModuleLine(lines, modules, "server-events");
            addModuleLine(lines, modules, "voicechat-sync");
        }
        ...
        if (python != null) {
            lines.add(line("Python", python.workerState() == com.hubertstudios.coredsc.scripting.PythonWorker.State.READY, python.workerDetail()));
        ...
        queueFuture.handle((counts, error) -> new AsyncResult(counts, error))
                ...
                .whenComplete((results, combinationError) -> plugin.runSync(() -> {
    ...
    public void fix(CommandSender sender) {
        ...
        if (queue != null) queue.retryFailed().whenComplete((count, error) -> plugin.runSync(() -> sender.sendMessage(
        ...
        if (plugin.getStorage() != null) new PendingLinkCodeRepository(plugin.getStorage())
                .deleteExpired(System.currentTimeMillis()).whenComplete((count, error) -> plugin.runSync(() -> sender.sendMessage(
    ...
    private void inspectDiscord(List<String> lines, DiscordBotService discord) {
        ...
        Guild guild = guildId <= 0L ? null : jda.getGuildById(guildId);
        lines.add(line("Configured guild", guild != null, guild == null ? "not visible/configured" : guild.getName()));
    ...
    private void testChannel(CommandSender sender, String channelId, String message) {
        ...
        if (queue != null) {
            queue.enqueue(channelId, message, 100, "doctor:" + channelId + ':' + System.currentTimeMillis())
                    .whenComplete((id, error) -> plugin.runSync(() -> sender.sendMessage(error == null
        ...
        channel.sendMessage(message).queue(ignored -> plugin.runSync(() -> sender.sendMessage("§aTest message sent.")),
                error -> plugin.runSync(() -> sender.sendMessage("§cTest failed: " + rootMessage(error))));
    }
