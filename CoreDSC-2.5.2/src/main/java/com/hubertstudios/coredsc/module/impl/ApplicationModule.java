...
public final class ApplicationModule implements CoreModule, DiscordCommandContributor {
    ...
    public void enable() {
        ...
        bukkitListener=new Listener(){@EventHandler public void onJoin(PlayerJoinEvent event){deliverOffline(event.getPlayer());}};
    ...
    private void registerMinecraftCommands(){
        ...
        staff.setExecutor((sender,cmd,label,args)->{if(!sender.hasPermission("coredsc.application.manage")){sender.sendMessage("§cNo permission.");return true;}if(args.length<2){sender.sendMessage("§e/application <view|accept|reject> <id> [note]");return true;}Long id=parseId(args[1]);if(id==null){sender.sendMessage("§cInvalid ID.");return true;}switch(args[0].toLowerCase(Locale.ROOT)){
            case "view"->applications.find(id).thenCombine(applications.answers(id),(app,answers)->app.map(a->format(a,answers)).orElse("Application not found.")).whenComplete((text,error)->plugin.runSync(()->sender.sendMessage(error==null?text:"§cLookup failed.")));
            case "accept","reject"->{boolean accept=args[0].equalsIgnoreCase("accept");String note=args.length>2?join(args,2):"";decide(id,accept,sender.getName(),note).whenComplete((ok,error)->plugin.runSync(()->sender.sendMessage(error==null&&ok?"§aApplication decided.":"§cApplication not found or already decided.")));}
            default->sender.sendMessage("§e/application <view|accept|reject> <id> [note]");
        }return true;});
    }

    private void start(Player p){links.findByMinecraftUuid(p.getUniqueId().toString()).thenCompose(link->{if(requireLinked&&link.isEmpty())return CompletableFuture.failedFuture(new IllegalStateException("Link your Discord account first."));return applications.start(p.getUniqueId().toString(),p.getName(),link.map(LinkedAccountRepository.LinkedAccount::discordUserId).orElse(""),System.currentTimeMillis());}).whenComplete((id,error)->plugin.runSync(()->{if(error!=null){p.sendMessage("§c"+rootMessage(error));return;}p.sendMessage("§aApplication #"+id+" started. Use §e/apply questions§a and §e/apply answer <id> <answer>§a.");}));}
    ...
    private void answer(Player p,String questionId,String answer){Question q=questions.stream().filter(x->x.id().equalsIgnoreCase(questionId)).findFirst().orElse(null);if(q==null){p.sendMessage("§cUnknown question ID.");return;}String clean=TextUtil.truncate(TextUtil.sanitizeMinecraftUserText(answer).trim(),q.maxLength());if(clean.length()<q.minLength()){p.sendMessage("§cAnswer is too short.");return;}applications.activeForUser(p.getUniqueId().toString()).thenCompose(app->app.isEmpty()?CompletableFuture.failedFuture(new IllegalStateException("Start an application first.")):applications.answer(app.get().id(),q.id(),clean,System.currentTimeMillis())).whenComplete((v,e)->plugin.runSync(()->p.sendMessage(e==null?"§aAnswer saved.":"§c"+rootMessage(e))));}
    private void submit(Player p){applications.activeForUser(p.getUniqueId().toString()).thenCompose(app->{
    ...
    }).whenComplete((v,e)->plugin.runSync(()->p.sendMessage(e==null?"§aApplication submitted.":"§c"+rootMessage(e))));}
    private void status(Player p){applications.activeForUser(p.getUniqueId().toString()).whenComplete((app,e)->plugin.runSync(()->p.sendMessage(e!=null?"§cLookup failed.":app.isEmpty()?"§7No active application.":"§bApplication #"+app.get().id()+" §8["+app.get().status()+"]")));}
    ...
    private CompletableFuture<Void> notifyDecision(Application application,String status,String by,String note){
        ...
        return plugin.callSync(()->Bukkit.getPlayer(UUID.fromString(application.minecraftUuid()))).thenCompose(online->
                messages.add("APPLICATION",application.id(),"SYSTEM","", "CoreDSC",text,System.currentTimeMillis(),online!=null,true)
                        .thenAccept(messageId->{if(online!=null)plugin.runSync(()->online.sendMessage("§b[Application #"+application.id()+"] §f"+text));}));
    }

    private void deliverOffline(Player player){
        messages.pendingForMinecraft(player.getUniqueId().toString(),"APPLICATION",50).whenComplete((pending,error)->{
            ...
            plugin.runSync(()->{List<Long>ids=new ArrayList<>();for(SupportMessage message:own){player.sendMessage("§b[Application #"+message.itemId()+"] §f"+message.message());ids.add(message.id());}messages.markMinecraftDelivered(ids);});
        });
    }
